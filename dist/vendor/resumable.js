  ...
  var Resumable = function(opts){
    ...
    $.defaults = {
      ...
      maxFilesErrorCallback:function (files, errorCount) {
        ...
        alert('一次只能上传'+maxFiles+'个文件');
      },
      minFileSize:1,
      minFileSizeErrorCallback:function(file, errorCount) {
        alert(file.fileName||file.name +' 太小，文件大小最小 ' + $h.formatSize($.getOpt('minFileSize')) + '');
      },
      maxFileSize:undefined,
      maxFileSizeErrorCallback:function(file, errorCount) {
        alert(file.fileName||file.name +' 太大，文件大小最大 ' + $h.formatSize($.getOpt('maxFileSize')) + '');
      },
      fileType: [],
      fileTypeErrorCallback: function(file, errorCount) {
        alert(file.fileName||file.name +' 文件类型不允许，允许上传的文件类型为 ' + $.getOpt('fileType') + '');
      }
    };
    $.opts = opts||{};
    $.getOpt = function(o) {
